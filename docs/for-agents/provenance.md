...
| `@appkit/reports` | substantial generalized extraction — domain catalogues and delivery transports remain application-owned | Fiscal periods, nested filters, row/summary SQL compilation, relationship refinement, custom-field metadata expressions, fiscal bins, grouped result shaping, definition registry, DST-safe cadence, schedule policy, durable idempotent run claiming, document contracts/rendering, and the interactive one-third/two-thirds report studio are present. Domain built-in definitions, tenant schema discovery, and email/object-storage transports are injected because they are application-owned. |
