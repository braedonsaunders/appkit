...
| `@appkit/reports` | substantial generalized extraction — viewer parity restored; studio and consumer cutover pending | The paper/table exports now retain source-compatible names and props. The production statement viewer behavior is present in full: grouped headings, hierarchy, accounting currency, scale, variance percentages, totals, collapse/expand, account navigation, typed drill targets, and the paged supporting-row drawer. The compact toolbar restores every source control through application-owned state. Stored queries use the shared production `ReportCustomQuery`/`op`/`fn` contract, and tests compile both source catalogue families without translation. The one-third/two-thirds studio retains ordered/renamable columns, recursive filters, three-level sorting, page setup, run/save, paper preview, and export, but the more complete source studio's template/search/autopreview behavior and a direct consuming-application import cutover remain required before calling the package drop-in complete. Product catalogues, SQL loaders, native record drawers, and export HTTP routes remain injected because they are application-owned. |
