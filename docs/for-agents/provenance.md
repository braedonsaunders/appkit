...
| `@appkit/reports` | substantial faithful generalized extraction — scheduled-UI and reference-consumer cutover pending | The paper/table exports retain source-compatible names and props. The production statement viewer behavior is present in full: grouped headings, hierarchy, accounting currency, scale, variance percentages, totals, collapse/expand, account navigation, typed drill targets, and the paged supporting-row drawer. The compact toolbar restores every source control through application-owned state. Stored queries use the shared production `ReportCustomQuery`/`op`/`fn` contract, and packed-consumer tests compile both source catalogue families without translation. The one-third/two-thirds studio now includes the more complete source behavior: grouped searchable catalogues, catalogue-derived templates, ordered/renamable and searchable columns, recursive filters, three-level synchronized sorting, page setup, debounced live preview/autosave, manual run/save, paper preview, and export. The source scheduled-report forms and a direct consuming-application import cutover remain required before calling the package drop-in complete. Product catalogues, SQL loaders, native record drawers, and export HTTP routes remain injected because they are application-owned. |
