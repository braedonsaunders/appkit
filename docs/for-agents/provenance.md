...
| `@appkit/forms` | faithful generalized production fill-runtime extraction; designer parity pending | Reference B's complete 4,778-line production fill renderer and its canvas, photo-state, upload, layout, and generated-copy support were copied before generalization. The source behavior remains: revision/sequence-safe autosave, unload beacons, lazy drafts, inline saves, server validation, guided steps, formulas/defaults/logic, entity refresh, hierarchy and data queries/aggregates, repeating rows, the complete native control set, media/photo analysis and annotation, responsive columns/canvas, read-only records, and submission. Routes, persistence, authorization-bound queries, upload transport, and photo analysis are typed host adapters. Static source-shaped rendering tests and the packed consumer exercise the generalized production entry. The existing controlled `FormDesigner` is not the complete 4,186-line source designer; overview, workflow, assignments, permissions, publishing, assisted authoring, and representative consumer cutover remain pending. |
...
| `FormDesigner` | partial controlled authoring composition — full source designer extraction pending | The present field library, canvas, inspectors, record configuration, and workflow controls are reusable, but the complete 4,186-line source shell—including overview, flows, assignments, permissions, publishing, assisted authoring, and its exact navigation/interaction composition—has not yet been ported. Do not claim source or drop-in parity. |
