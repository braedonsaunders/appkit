...
| `@appkit/reports` | partial generalized extraction — source viewer parity pending | Fiscal periods, nested filters, SQL compilation, grouped result shaping, scheduling, durable run claiming, and document contracts are present. The current studio has not yet preserved reference A's paper presentation, complete report controls, drilldown navigation, and viewer/print interaction, so it must not be treated as source-parity or a drop-in replacement until that literal extraction is complete. |
