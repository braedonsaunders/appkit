...
| `@appkit/customization` | faithful generalized package, persistence, authoring, and consuming-list extraction — consuming-app cutover pending | Reference A's complete types, registry, Zod schemas, parse/lint helpers, locked/versioned merge behavior, default refresh logic, and upstream tests were copied before generalization. `RecordListView`, `EntityListView`, `ViewsMenu`, `FilterChips`, sortable headers, paging, effective-view resolution, live-column merge, and the four form/list preference tables are now represented by generalized package-owned contracts, React composition, memory repository, and tenant-scoped Drizzle repository. The form, list-view, and custom-field editors retain source groups, field placement, line columns, actions, filters, sorting, scope, field options/behavior, and save/delete flows. Currency, status meaning, routing, permission decisions, record labels, roles, and domain queries remain injected application concerns. Representative consuming-app compile cutover is still required before claiming import-level drop-in compatibility. |
