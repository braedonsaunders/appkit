...
| `@appkit/reports` | substantial faithful generalized extraction — reference-consumer cutover pending | The reference report builder and viewer now supply the one-third/two-thirds studio, ordered/renamable columns, recursive filters, three-level sorting, fiscal period operators, shared paper presentation, document tables and totals, collapsible statement matrix, compact filter bar, typed drill targets, paged supporting-row drawer, native-record seam, PDF-backed print, and PDF/XLSX/CSV action surface. AppKit retains the existing tenant-bound compiler, grouped results, page setup, document output, scheduling, and durable run claims. Product report catalogues, SQL loaders, record drawers, and export HTTP routes are injected rather than bundled. A direct consuming-application import cutover remains required before claiming drop-in completion. |
...
| `@appkit/iam` | substantial faithful generalized extraction — reference-consumer cutover pending | Reference B `admin/roles` list/detail/actions, permission matrix, scoped role-member manager, `admin/users` lifecycle/scope/override model, and IAM schema are combined with reference A `RoleEditor`, role list/API protections, permission deny semantics, user actions, and the complete audit list/detail drawer. AppKit retains searchable role/member/audit administration, built-in/current-user/super-admin protections, duplicate/delete, invitation persistence, scoped assignment upsert, per-user grant/deny overrides, before/after audit, dependency-free contracts, and tenant-bound Drizzle, authenticated HTTP, and memory adapters. Product permission catalogues, hierarchy choices, invitation credential delivery, and route authorization are typed application inputs. Representative unmodified reference consumer compilation and source-to-AppKit screenshot comparison remain required before a drop-in claim. |
...
| `@appkit/apps` | substantial faithful generalized extraction — consuming-app route cutover pending | Reference A `engine/src/apps-runtime.ts`, `schema/src/apps.ts`, `web/lib/apps/{manifest,zip,objects,bridge,store}.ts`, installed App frame, marketplace/library, API lifecycle, and `web/app/(app)/admin/apps/AppDrawer.tsx`. The source collapsible folder tree, file-type icons, CodeMirror JavaScript/HTML/CSS/JSON modes, line numbers/folding/completion, binary inspector, create/upload/delete/save controls, overview, and run inspection are preserved; AppKit additionally retains its governed live preview. Manifest/ZIP validation, immutable versions, object provisioning transaction seam, storage, capability intersection, QuickJS endpoints, opaque-origin iframe/CSP/SDK, publishing, memory/Drizzle persistence, and React authoring are present. `/service` and `createAppEndpointRuntime` provide tested source-shaped lifecycle and endpoint cutover seams with application-supplied global, scope mapping, permissions, records, and governed writes. Authentication, routes, and domain object schemas remain injected. |
...
| `@appkit/customization` | faithful generalized package and React extraction — consuming-app cutover pending | Reference A's complete types, registry, Zod schemas, parse/lint helpers, locked/versioned merge behavior, default refresh logic, and upstream tests were copied before generalization. Its form, list-view, and custom-field editors were then moved into the package with the source interaction model retained: groups, field placement, line columns, actions, filters, sorting, scope, field options/behavior, and save/delete flows. `CustomizationStudio` packages the complete library/editor composition, while the individual editors remain importable. Framework routing, authorization, tenant persistence, record labels, and role catalogues are injected adapters. Representative consuming-app compile cutover is still required before claiming import-level drop-in compatibility. |
