...
| `@appkit/jobs` | substantial generalized extraction — notification queues faithfully extracted; other product profiles remain | Lazy BullMQ connections, bounded readiness, source payload guards, atomic Redis rate limits, and hardened Web Push delivery are present. Reference B `packages/jobs/src/queues/{notify,push}.ts` is retained at `/notifications`: unchanged data contracts, bounds, normalization, queue names, deterministic batching, ids, retry/backoff, and retention are bound to the generalized Jobs runtime. Other application queue payload catalogues and schedules remain app-owned. |
