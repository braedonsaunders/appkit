...
const STATUS_VARIANT: Record<User['status'], BadgeProps['variant']> = {
  Active: 'success',
  Invited: 'info',
  Suspended: 'warning',
}
  ...
  return (
    <div>
      ...
      >
        ...
        {active === 'users' ? <UsersSettings /> : null}
...
function NavigationSettings() {
  const [visible, setVisible] = React.useState<Record<string, boolean>>(
    Object.fromEntries(NAV_ITEMS.map((n) => [n, true])),
  )
  return (
    <SettingsSection
      title="Sidebar navigation"
      description="Toggle which items appear in the main navigation."
    >
      {NAV_ITEMS.map((item) => (
        <SettingsRow
          key={item}
        ...
        >
          <Switch
            checked={!!visible[item]}
            onChange={(e) => setVisible((v) => ({ ...v, [item]: e.target.checked }))}
          />
        </SettingsRow>
      ))}
    </SettingsSection>
