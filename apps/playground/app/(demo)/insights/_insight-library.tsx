...
import type { CardStudioResult, InsightCardDraft } from '@appkitjs/dashboard'
import { CardStudio } from '@appkitjs/dashboard/react'
import {
...
export function InsightLibrary({ cards: initialCards, catalog, selectedId, createNew, browserPersistence }: { cards: InsightCardDraft[]; catalog: AnalyticsCatalog; selectedId: string | null; createNew: boolean; browserPersistence: boolean }) {
  ...
  return <>
    ...
    <Drawer open={!!selected} onClose={close} title={selected?.name ?? 'Insight card'} description="Card studio" size="2xl" initialFullscreen={createNew} bodyClassName="flex min-h-0 p-0">
      {selected ? <CardStudio key={selected.id ?? 'new'} initial={selected} catalog={catalog} onRun={runInsightQueryAction} onSave={async (draft) => { const result = await saveCard(draft); if (result.ok && result.id && !draft.id) { const saved = { ...draft, id: result.id }; cardsRef.current = [...cardsRef.current.filter((card) => card.id !== result.id), saved]; setCards(cardsRef.current); router.replace(`/insights?card=${result.id}`); if (!browserPersistence) router.refresh() } return result }} onDelete={async (draft) => { const result = await deleteCard(draft); if (result.ok) { close(); if (!browserPersistence) router.refresh() } return result }} onPublishChange={async (published, draft) => { const result = await publishCard(published, draft); if (result.ok && !browserPersistence) router.refresh(); return result }} /> : null}
    </Drawer>
  </>
}

function validStoredCard(value: InsightCardDraft): boolean {
  const visualizations = new Set(['scalar', 'progress', 'table', 'pivot', 'heatmap', 'bar', 'row', 'line', 'area', 'combo', 'pie', 'donut', 'funnel', 'gauge', 'scatter'])
  return Boolean(
    value
