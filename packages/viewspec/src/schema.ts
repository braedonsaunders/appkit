...
import { z } from 'zod'
import { SPEC_VERSION, type PageSpec } from './types'

/**
 * A field reference: a dot path into the loader's output, and nothing else.
 *
 * The forbidden segments are ALSO checked here, not only in `resolvePath`.
 * The resolver's guard is the one that must never be removed — it holds for
 * every caller, including a compiled spec that renders `trusted` and skips
 * validation entirely. But catching it only there means a poisoned path can be
 * STORED, and then throws for every reader who opens that page until someone
 * works out why. Refusing it at the edge tells whoever wrote it, once, while
...
const fieldRefSchema = z.strictObject({
  $: z
    ...
    .refine(
      ...
      'field path may not name __proto__, constructor, or prototype',
    ),
})
...
export const pageSpecSchema = z.strictObject({
  // The route PATTERN, not a url: `/items/[id]`, never `/items/42`. A
