...
test("buildScheduleInsights flags deadline, constraint, actual, and resource loading issues", () => {
  ...
  const assignments: ScheduleTaskAssignment[] = [
    {
      id: "asg-1", taskId: "task-a", resourceId: "res-1", units: 1, role: "Lead",
    },
    {
      id: "asg-2", taskId: "task-b", resourceId: "res-1", units: 1, role: "Lead",
    },
  ];
  const tasks = [
    makeTask({
...
test("computePhaseDatesFromTasks returns the earliest start and latest finish per phase", () => {
  const phases: SchedulePhase[] = [
    { id: "phase-1", number: "1", name: "Preconstruction", description: "", order: 1, startDate: null, endDate: null, color: "" },
  ];
...
test("calendars skip weekends and honour holiday exceptions", () => {
  const calendar: ScheduleCalendar = {
    id: "cal-1",
    name: "Standard",
    workingDays: { "0": false, "1": true, "2": true, "3": true, "4": true, "5": true, "6": false },
  ...
  // 2026-04-03 is a Friday; +1 working day skips the weekend AND the Monday holiday.
  assert.equal(formatISODate(addWorkingDays(parseDate("2026-04-03")!, 1, calendar)), "2026-04-07");
  assert.equal(isNonWorkingDay(parseDate("2026-04-06")!, calendar), true);
  assert.equal(isNonWorkingDay(parseDate("2026-04-07")!, calendar), false);
  // Mon–Fri across a two-week span, less the holiday.
  assert.equal(
    countWorkingDays(parseDate("2026-04-06")!, parseDate("2026-04-20")!, calendar),
    9,
  );
});

test("levelResources delays the floaty task when two tasks share one crew", () => {
  const resources: ScheduleResource[] = [
    { id: "res-1", name: "Crew A", kind: "crew", defaultUnits: 1, capacityPerDay: 1 },
  ];
  const assignments: ScheduleTaskAssignment[] = [
    { id: "asg-1", taskId: "critical", resourceId: "res-1", units: 1 },
    { id: "asg-2", taskId: "floaty", resourceId: "res-1", units: 1 },
  ];
  const tasks = [
    // critical → successor keeps the chain at zero float
    makeTask({ id: "critical", order: 1, startDate: "2026-04-06", endDate: "2026-04-09", duration: 3 }),
    makeTask({ id: "successor", order: 2, startDate: "2026-04-09", endDate: "2026-04-20", duration: 11 }),
    makeTask({ id: "floaty", order: 3, startDate: "2026-04-06", endDate: "2026-04-09", duration: 3 }),
  ];
  const dependencies: ScheduleDependency[] = [
    { id: "dep-1", predecessorId: "critical", successorId: "successor", type: "FS", lagDays: 0 },
  ];

  const result = levelResources(tasks, dependencies, {
    resources,
    assignments,
    withinFloatOnly: false,
  });

  assert.equal(result.hasCycle, false);
  assert.equal(result.moves.length, 1);
  // The zero-float task holds its dates; the one with slack absorbs the delay.
  assert.equal(result.moves[0]!.taskId, "floaty");
  assert.ok(result.moves[0]!.delayDays > 0);
  assert.deepEqual(result.moves[0]!.blockedByResourceIds, ["res-1"]);

  const leveled = applyLevelingMoves(tasks, result.moves);
  const leveledFloaty = leveled.find((task) => task.id === "floaty")!;
  assert.equal(leveledFloaty.startDate, result.moves[0]!.toStart);
});

test("levelResources reports what it could not solve inside available float", () => {
  const resources: ScheduleResource[] = [
    { id: "res-1", name: "Crew A", kind: "crew", defaultUnits: 1, capacityPerDay: 1 },
  ];
  const assignments: ScheduleTaskAssignment[] = [
    { id: "asg-1", taskId: "task-a", resourceId: "res-1", units: 1 },
    { id: "asg-2", taskId: "task-b", resourceId: "res-1", units: 1 },
  ];
  // Two zero-float tasks booking the same single-capacity crew on the same days.
  const tasks = [
    makeTask({ id: "task-a", order: 1, startDate: "2026-04-06", endDate: "2026-04-10", duration: 4 }),
    makeTask({ id: "task-b", order: 2, startDate: "2026-04-06", endDate: "2026-04-10", duration: 4 }),
  ];

  const result = levelResources(tasks, [], { resources, assignments, withinFloatOnly: true });

  assert.equal(result.moves.length, 0);
  assert.equal(result.unresolved.length, 1);
  assert.equal(result.unresolved[0]!.taskId, "task-b");
  assert.equal(result.unresolved[0]!.reason, "exceeds_float");
  assert.ok(result.unresolved[0]!.requiredDelayDays > 0);
});

test("levelResources leaves started work in place and plans around it", () => {
  const resources: ScheduleResource[] = [
    { id: "res-1", name: "Crew A", kind: "crew", defaultUnits: 1, capacityPerDay: 1 },
  ];
  const assignments: ScheduleTaskAssignment[] = [
    { id: "asg-1", taskId: "in-flight", resourceId: "res-1", units: 1 },
    { id: "asg-2", taskId: "planned", resourceId: "res-1", units: 1 },
  ];
  const tasks = [
    makeTask({
      id: "in-flight",
    ...
    makeTask({ id: "planned", order: 2, startDate: "2026-04-06", endDate: "2026-04-10", duration: 4 }),
  ];
  ...
  const result = levelResources(tasks, [], {
    ...
    assignments,
    withinFloatOnly: false,
    freezeStartedTasks: true,
  });

  assert.deepEqual(
    result.moves.map((move) => move.taskId),
    ["planned"],
  );
});

test("buildResourceLoadSeries reports daily load against capacity", () => {
  const calendar: ScheduleCalendar = {
    id: "cal-1",
    name: "Standard",
    workingDays: { "0": false, "1": true, "2": true, "3": true, "4": true, "5": true, "6": false },
  ...
  const resources: ScheduleResource[] = [
    { id: "res-1", name: "Crew A", kind: "crew", calendarId: "cal-1", defaultUnits: 1, capacityPerDay: 1 },
  ];
  const assignments: ScheduleTaskAssignment[] = [
    { id: "asg-1", taskId: "task-a", resourceId: "res-1", units: 1 },
    { id: "asg-2", taskId: "task-b", resourceId: "res-1", units: 1 },
  ...
  const tasks = [
    makeTask({ id: "task-a", order: 1, startDate: "2026-04-06", endDate: "2026-04-08" }),
    makeTask({ id: "task-b", order: 2, startDate: "2026-04-07", endDate: "2026-04-09" }),
  ];

  const [series] = buildResourceLoadSeries(tasks, { calendars: [calendar], resources, assignments });

  assert.ok(series);
  assert.equal(series!.capacityPerDay, 1);
  assert.equal(series!.peakLoad, 2);
  assert.equal(series!.overloadedDays, 1);
  assert.deepEqual(
    series!.days.map((day) => [day.date, day.load]),
    [
      ["2026-04-06", 1],
      ["2026-04-07", 2],
      ["2026-04-08", 1],
...
test("mergeSchedulingLabels overlays only what a host overrides", () => {
  const labels = mergeSchedulingLabels({ status: { complete: "Terminé" } });
  assert.equal(labels.status.complete, "Terminé");
  assert.equal(labels.status.not_started, defaultSchedulingLabels.status.not_started);
  assert.equal(labels.view.gantt, defaultSchedulingLabels.view.gantt);
});
