...
import { Button, Checkbox, Input, Label, SearchSelect, Textarea, cn } from '@braedonsaunders/ui'
...
import { ReportDocumentView } from './document-view'
...
export function ReportStudio<TDrillTarget = never, TRecord extends ReportDrillRecord = ReportDrillRecord>({ value, catalog, result, onChange, onPreview, onSave, onSaved, onDelete, onDeleted, organization = 'Organization', logoUrl, primaryColor, currency = '', drill, exports: exportOptions, printHref, pdfHref, templates, autoPreviewMs = 500, autoSaveMs = 700, className }: {
  ...
  async function save(next = value): Promise<boolean> {
    ...
    try {
      ...
      if (response.value) onChange(response.value)
...
function RowsBuilder({ entity, query, onChange }: { entity: NonNullable<ReturnType<typeof reportEntity>>; query: ReportCustomQuery; onChange: (query: ReportCustomQuery) => void }) {
  ...
  return <BuilderSection title="Columns" icon={<Columns3 />}>
    ...
    <Field label="Group rows into sections"><SearchSelect value={query.groupBy ?? ''} onChange={(groupBy) => onChange({ ...query, groupBy: groupBy || null })} options={[{ value: '', label: 'No grouping' }, ...columns.map((column) => ({ value: column.key, label: column.label }))]} /></Field>
  </BuilderSection>
}
...
function SummaryBuilder({ entity, query, onChange }: { entity: NonNullable<ReturnType<typeof reportEntity>>; query: ReportCustomQuery; onChange: (query: ReportCustomQuery) => void }) {
  ...
  const columns = visibleReportColumns(entity)
  return <><BuilderSection title="Group by" icon={<GripVertical />} action={() => onChange({ ...query, breakouts: [...breakouts, { column: columns[0]?.key ?? '' }] })}>{breakouts.map((breakout, index) => { const column = reportColumn(entity, breakout.column); const temporal = column?.kind === 'date' || column?.kind === 'timestamp'; return <div key={index} className="flex gap-2"><SearchSelect className="min-w-0 flex-1" value={breakout.column} onChange={(columnKey) => onChange({ ...query, breakouts: breakouts.map((item, itemIndex) => itemIndex === index ? { ...item, column: columnKey, bin: undefined } : item) })} options={columns.map((item) => ({ value: item.key, label: item.label }))} />{temporal ? <SearchSelect className="w-32" value={breakout.bin ?? ''} onChange={(bin) => onChange({ ...query, breakouts: breakouts.map((item, itemIndex) => itemIndex === index ? { ...item, bin: bin ? bin as typeof item.bin : undefined } : item) })} options={[{ value: '', label: 'Exact' }, ...REPORT_TEMPORAL_BINS.map((bin) => ({ value: bin, label: bin.replace('_', ' ') }))]} /> : null}<RemoveButton onClick={() => onChange({ ...query, breakouts: breakouts.filter((_, itemIndex) => itemIndex !== index) })} /></div> })}</BuilderSection>
    <BuilderSection title="Measures" icon={<Sigma />} action={() => onChange({ ...query, measures: [...measures, { fn: 'count' }] })}>{measures.map((measure, index) => <div key={index} className="flex gap-2"><SearchSelect className="w-32" value={measure.fn} onChange={(fn) => onChange({ ...query, measures: measures.map((item, itemIndex) => itemIndex === index ? { ...item, fn: fn as ReportAggFn, column: fn === 'count' ? undefined : item.column ?? columns.find((column) => column.kind === 'number' || column.kind === 'money')?.key } : item) })} options={REPORT_AGG_FNS.map((fn) => ({ value: fn, label: fn.replace('_', ' ') }))} />{measure.fn !== 'count' ? <SearchSelect className="min-w-0 flex-1" value={measure.column ?? ''} onChange={(column) => onChange({ ...query, measures: measures.map((item, itemIndex) => itemIndex === index ? { ...item, column } : item) })} options={columns.filter((column) => !['sum', 'avg'].includes(measure.fn) || column.kind === 'number' || column.kind === 'money').map((column) => ({ value: column.key, label: column.label }))} /> : <div className="flex flex-1 items-center rounded-md border border-border bg-bg-subtle px-3 text-sm text-fg-muted">Rows</div>}<RemoveButton onClick={() => onChange({ ...query, measures: measures.filter((_, itemIndex) => itemIndex !== index) })} /></div>)}</BuilderSection></>
}
...
function SortLimitBuilder({ entity, query, onChange }: { entity: NonNullable<ReturnType<typeof reportEntity>>; query: ReportCustomQuery; onChange: (query: ReportCustomQuery) => void }) {
  ...
  return <BuilderSection title="Rows" icon={<Table2 />}>
    <div className="space-y-2"><div className="flex items-center justify-between"><Label>Sort by</Label>{sorts.length > 0 && sorts.length < 3 ? <Button type="button" variant="ghost" size="sm" onClick={() => { const first = columns.find((column) => !used.has(column.key)); if (first) commit([...sorts, { column: first.key, direction: 'desc' }]) }}><Plus size={14} />Add level</Button> : null}</div>
      {sorts.length === 0 ? <SearchSelect value="" onChange={(column) => column && commit([{ column, direction: 'desc' }])} options={[{ value: '', label: 'Default order' }, ...columns.map((column) => ({ value: column.key, label: column.label }))]} /> : sorts.map((sort, index) => <div key={index} className="flex items-center gap-2">{index > 0 ? <span className="w-12 shrink-0 text-right text-[11px] text-fg-subtle">then by</span> : null}<SearchSelect className="min-w-0 flex-1" value={sort.column} onChange={(column) => commit(sorts.map((item, itemIndex) => itemIndex === index ? { ...item, column } : item))} options={columns.filter((column) => !used.has(column.key) || column.key === sort.column).map((column) => ({ value: column.key, label: column.label }))} /><SearchSelect className="w-32" value={sort.direction} onChange={(direction) => commit(sorts.map((item, itemIndex) => itemIndex === index ? { ...item, direction: direction as 'asc' | 'desc' } : item))} options={[{ value: 'desc', label: 'Descending' }, { value: 'asc', label: 'Ascending' }]} /><RemoveButton onClick={() => commit(sorts.filter((_, itemIndex) => itemIndex !== index))} /></div>)}</div>
...
export function reportStudioTemplates(entity: NonNullable<ReturnType<typeof reportEntity>>): ReportStudioTemplate[] {
  ...
  const numeric = visibleColumns.find((column) => column.kind === 'number' || column.kind === 'money')
...
function reportStudioPersistenceKey(value: ReportStudioValue): string {
