...
// `unseal`, e.g. @braedonsaunders/appkit-crypto's unsealSecret) into an `SmsTransport`;
...
export type PlatformSmsConfig = RawSmsConfig & { mode?: SmsPolicyMode }

export type PlainSmsConfig = Omit<RawSmsConfig, 'keyCiphertext' | 'keyNonce'> & { secret?: string }
...
function validateSmsConfigFields(raw: PlainSmsConfig | RawSmsConfig, requireComplete: boolean): void {
  ...
  if ('secret' in raw && !(raw as PlainSmsConfig).secret?.trim()) {
...
export function resolveSmsTransport(raw: RawSmsConfig | null | undefined, unseal: Unseal): SmsTransport | null {
...
async function readBoundedText(response: Response): Promise<string> {
  ...
  if (received === MAX_SMS_RESPONSE_BYTES) await reader.cancel().catch(() => undefined)
...
async function requestJson(
  url: string,
  options: { headers: Record<string, string>; body: string | URLSearchParams },
...
export async function sendSmsVia(transport: SmsTransport, input: SendSmsInput): Promise<{ id: string }> {
...
async function sendMessagebird(
  ...
  const json = responseRecord(body) as { id?: string; errors?: { description?: string }[] } | null
