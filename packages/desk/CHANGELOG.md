...
- 36dad78: Desks can now be parked with their memory, so resuming continues the guest's process tree instead of cold booting it.
...
- 363a854: Recover screen capture after a guest-agent reconnect by invalidating stale running state while preserving the existing screen handle and handover mask. Preserve native double-clicks as one validated guest request instead of two latency-sensitive round trips.

## 0.5.0

### Minor Changes

- a931ed5: Desk overlays are reclaimed, and they stop ratcheting.
