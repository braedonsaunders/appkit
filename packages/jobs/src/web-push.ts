...
import { resolvePublicHost, secureFetch, validateOutboundRequestConfiguration } from '@braedonsaunders/appkit-egress-proxy/secure-fetch'
...
export type WebPushSubscriptionInput = { endpoint: string; keys: { p256dh: string; auth: string } }
export type WebPushPayload = { title: string; body?: string; linkPath?: string }
export type WebPushVapidDetails = { subject: string; publicKey: string; privateKey: string }
...
function decodeBase64Url(value: string, label: string, expectedBytes: number): string {
  ...
  if (value.length > maxEncodedLength) throw new Error(`${label} has an invalid length or encoding.`)
  ...
  if (!normalized || [...normalized].some((character) => !/[A-Za-z0-9_-]/.test(character))) throw new Error(`${label} is not valid base64url data.`)
  const decoded = Buffer.from(normalized, 'base64url')
  if (decoded.length !== expectedBytes || decoded.toString('base64url') !== normalized) throw new Error(`${label} has an invalid length or encoding.`)
  return normalized
}

export function validateWebPushEndpoint(endpoint: string): string {
  if (typeof endpoint !== 'string' || !endpoint || endpoint.length > MAX_ENDPOINT_LENGTH) throw new Error('Web Push endpoint is missing or too long.')
...
export function validateWebPushSubscription(input: WebPushSubscriptionInput): WebPushSubscriptionInput {
  ...
  if (Buffer.from(p256dh, 'base64url')[0] !== 4) throw new Error('Web Push p256dh key is not an uncompressed P-256 key.')
  const auth = decodeBase64Url(input.keys.auth, 'Web Push auth secret', 16)
  return { endpoint, keys: { p256dh, auth } }
}

export async function validateWebPushSubscriptionForPersistence(input: WebPushSubscriptionInput): Promise<WebPushSubscriptionInput> {
...
function truncateJsonString(value: string, maxEncodedBytes: number): string {
  ...
  while (low < high) {
    ...
    if (Buffer.byteLength(JSON.stringify(points.slice(0, middle).join(''))) <= maxEncodedBytes) low = middle
    else high = middle - 1
  }
  if (low === points.length) return value
  while (low > 0 && Buffer.byteLength(JSON.stringify(`${points.slice(0, low).join('')}…`)) > maxEncodedBytes) low -= 1
...
export function buildWebPushPayload(input: WebPushPayload): string {
  ...
  while (low < high) {
    ...
    if (Buffer.byteLength(candidate) <= MAX_PUSH_PAYLOAD_BYTES) low = middle
    else high = middle - 1
  }
  const payload = JSON.stringify({ ...base, body: `${points.slice(0, low).join('')}${low < points.length ? '…' : ''}` })
  if (Buffer.byteLength(payload) > MAX_PUSH_PAYLOAD_BYTES) throw new Error('Web Push payload could not be reduced to the delivery limit.')
  return payload
}
...
export async function sendWebPushNotification(input: { subscription: WebPushSubscriptionInput; payload: WebPushPayload; vapid: WebPushVapidDetails }): Promise<void> {
  ...
  const error = new Error(`Web Push provider returned HTTP ${response.status}${detail ? `: ${detail}` : ''}`) as Error & { statusCode?: number }
