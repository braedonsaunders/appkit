...
// One job per (automation, event): isolated retries never re-fire successful destinations.
...
export function createOutboundQueue(jobs: Jobs, overrides: QueueProfileOverrides = {}) {
  let queue: ReturnType<typeof createProfileQueue<OutboundDispatchJob>> | undefined
  const getQueue = () => queue ??= createProfileQueue<OutboundDispatchJob>(jobs, OUTBOUND_QUEUE_PROFILE, overrides)
  return {
    getQueue,
    async enqueueOutboundDispatch(data: OutboundDispatchJob, jobId?: string) {
    ...
    createWorker<R>(processor: Processor<OutboundDispatchJob, R>) {
