  ...
  const src =
    element.source === 'tenant.logo'
      ...
      : element.source === 'recipient.photo'
        ...
        : element.source === 'field' || element.field
          ? valueForField(element.field ?? '', data)
          : element.url
...
function renderQr(
  ...
  const qrData = valueForField(element.field, data)
  if (qrData) {
    return `<img class="ds-el" src="${esc(qrData)}" alt="" style="${style}background:${esc(element.background ?? '#fff')};padding:0.03in;"/>`
  }
...
function renderSeal(
  ...
  const tenantName = typeof data.tenantName === 'string'
    ? data.tenantName
    : valueForField('tenant.name', data) || valueForField('organization.name', data)
