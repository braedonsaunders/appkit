...
import { artboardSizeForFormat } from './schema'
import { defaultPrintProfile } from './print'
...
export function createDesignDocument(input: {
  ...
  const theme = { ...DEFAULT_DESIGN_STUDIO_THEME, ...input.theme }
  ...
  return {
    ...
    artboards: [{
      ...
      printProfile: defaultPrintProfile(format.startsWith('cr80') ? 'cr80' : format === 'custom' ? 'custom' : 'letter'),
      elements: [],
    }],
  }
}
