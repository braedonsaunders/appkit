...
export function validateDesignDocument(
  ...
  if (document.artboards.length === 0) errors.push('A design requires at least one artboard')
  ...
  for (const artboard of document.artboards) {
    ...
    for (const element of artboard.elements) {
      ...
      ids.add(element.id)
      const field =
        element.kind === 'field' || element.kind === 'qr'
          ? element.field
          : element.kind === 'image' && element.source === 'field'
            ? element.field
            : undefined
      if (field && knownFields && !knownFields.has(field)) errors.push(`Unknown data field: ${field}`)
    }
  }
  return errors
}
