...
import { Badge, Button, Input, Select, Textarea, cn } from '@braedonsaunders/appkit-ui'
  ...
  return (
    <>
      ...
      <button
        ...
        className="w-12 rounded px-1 py-1 text-center text-xs font-medium text-fg-muted tabular-nums hover:bg-surface-muted-subtle"
      >
...
export function ArtboardCanvas({
  ...
  return (
    <div
      className="rounded-md bg-surface-muted p-8 shadow-inner"
      style={{
        backgroundImage:
  ...
  return (
    <div className="space-y-3">
      ...
      <ElementButton
        ...
        onClick={() => onAdd('seal')}
      />
      <div className="rounded-md border border-border bg-surface-subtle p-3 text-xs leading-5 text-fg-muted">
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-md border border-border bg-surface-subtle p-3">
        ...
        <p className="mt-1 text-xs leading-5 text-fg-muted-subtle">
        ...
        value={
          selectedElement.kind === 'field' ? (
            <>
              ...
              <div className="rounded-md border border-primary bg-primary-subtle/60 px-2.5 py-2 text-xs text-primary">
        ...
        value={
          'color' in selectedElement ? (
            <ColorField
              ...
              value={selectedElement.color ?? hexColor('fg')}
        ...
        value={
          'fill' in selectedElement ? (
            <ColorField
              ...
              value={selectedElement.fill ?? hexColor('surface')}
        ...
        value={
          'stroke' in selectedElement ? (
            <ColorField
              ...
              value={selectedElement.stroke ?? hexColor('border-strong')}
...
export function RailLabel({ icon, label }: { icon: ReactNode; label: string }) {
  return (
    <div className="flex items-center gap-1.5 text-[11px] font-semibold tracking-wider text-fg-muted uppercase-subtle">
      <GeneratedValue value={icon} />
...
export function Field({ label, children }: { label: string; children: ReactNode }) {
  return (
    <label className="block space-y-1.5">
      <span className="text-xs font-medium text-fg-muted-subtle">
        <GeneratedValue value={label} />
      </span>
...
function ElementButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex w-full items-center gap-2 rounded-md border border-border bg-surface px-3 py-2 text-left text-sm font-medium text-fg-muted transition-colors hover:bg-surface-subtle"
    >
      <GeneratedValue value={icon} />
      <GeneratedValue value={label} />
    </button>
...
export function LayerToggle({
  ...
  return (
    <label className="flex cursor-pointer items-center justify-between rounded-md border border-border bg-surface px-2.5 py-2 text-sm">
...
function ColorField({
  ...
  return (
    <label className="flex items-center gap-2">
      <span className="w-20 text-xs font-medium text-fg-muted-subtle">
        <GeneratedValue value={label} />
      </span>
      <input
        type="color"
        ...
        className="h-8 w-10 rounded border border-border bg-surface p-0.5"
      />
      <Input
        value={value}
        maxLength={11}
...
export function newElement(
  ...
  if (kind === 'seal') return { ...base, kind, fill: hexColor('warning'), stroke: hexColor('primary'), text: '' }
  if (kind === 'ellipse')
...
export function createDesignElement(
  ...
  if (element.kind === 'image') return { ...element, source: defaultImage ? 'field' : element.source, field: defaultImage }
  if (element.kind === 'qr') return { ...element, field: defaultQr, background: theme.paper, foreground: theme.ink }
  if (element.kind === 'seal') return { ...element, fill: theme.accent, stroke: theme.primary }
  if (element.kind === 'ellipse' || element.kind === 'rect') return { ...element, fill: theme.paper, stroke: theme.primary }
  if (element.kind === 'line') return { ...element, stroke: theme.ink }
  return element
}
