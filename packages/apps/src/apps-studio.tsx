...
import { Badge, Button, Drawer, Input, Label, Select, Table, TableBody, TableCell, TableHead, TableHeader, TableRow, Textarea, cn } from '@braedonsaunders/appkit-ui'
...
function AppEditorDrawer({ app, files, runs, capabilities, context, onClose, onSaveMeta, onSaveFile, onDeleteFile, onStatusChange, onDelete, onPublish, onBridgeCall }: {
  ...
  const [meta, setMeta] = React.useState(() => ({ name: app.name, description: app.description ?? '', iconKey: app.iconKey, version: app.version ?? '0.1.0', showInNav: app.showInNav, requestedPermissions: [...(app.manifest?.permissions ?? [])], grantedPermissions: [...app.grantedPermissions], networkOrigins: [...(app.manifest?.network?.origins ?? [])] }))
    ...
    <div key={tab} className="min-h-0 flex-1 overflow-y-auto p-1 pt-4">
      {tab === 'overview' ? <div className="mx-auto max-w-3xl space-y-6">
        ...
        <section className="space-y-3">
          <div className="flex items-start justify-between gap-3"><div><h3 className="font-semibold text-fg">Public data sources</h3><p className="text-sm text-fg-muted">Declare exact HTTPS origins here. A host capability grant still controls whether backend code may request them.</p></div><Button type="button" size="sm" variant="outline" onClick={() => setMeta((current) => ({ ...current, networkOrigins: [...current.networkOrigins, 'https://'] }))}><Plus className="size-4" /> Add source</Button></div>
          {meta.networkOrigins.length ? <div className="space-y-2">{meta.networkOrigins.map((origin, index) => <div key={index} className="flex gap-2"><Input aria-label={`Public data source ${index + 1}`} value={origin} placeholder="https://api.example.com" onChange={(event) => setMeta((current) => ({ ...current, networkOrigins: current.networkOrigins.map((value, candidate) => candidate === index ? event.currentTarget.value : value) }))} /><Button type="button" size="icon" variant="ghost" aria-label={`Remove public data source ${index + 1}`} onClick={() => setMeta((current) => ({ ...current, networkOrigins: current.networkOrigins.filter((_, candidate) => candidate !== index) }))}><Trash2 className="size-4" /></Button></div>)}</div> : <div className="rounded-lg border border-dashed border-border px-4 py-6 text-center text-sm text-fg-muted">No public data sources declared.</div>}
        </section>
