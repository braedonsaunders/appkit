...
function parseConfig(config: Record<string, unknown>): HttpConfig {
  ...
  return {
    ...
    contentType:
      String(config.contentType ?? 'application/json').trim() ||
      'application/json',
    authHeaderName:
      String(config.authHeaderName ?? 'Authorization').trim() ||
      'Authorization',
  }
}
...
async function deliver(ctx: DeliverContext): Promise<DeliverResult> {
  ...
  if (batch && ctx.items.length > 0) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        return {
          ok: false,
          error: `HTTP ${res.status}${text ? `: ${text.slice(0, 140)}` : ''}`,
        }
      }
      return {
        ok: true,
  ...
  for (let index = 0; index < ctx.items.length; index++) {
    ...
    try {
      ...
      if (!res.ok) {
        const text = await res.text().catch(() => '')
        errors.push(
          `HTTP ${res.status}${text ? `: ${text.slice(0, 140)}` : ''}`,
        )
        continue
      }
  ...
  const resumeNote =
    resumed > 0 ? `; ${resumed} already succeeded on an earlier attempt` : ''
