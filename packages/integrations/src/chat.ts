...
async function test(ctx: DestinationTestContext): Promise<IntegrationResult> {
  ...
  try {
    const r = await post(url, { text: 'AppKit connection test ✅' })
    return r.ok
...
async function deliver(ctx: DeliverContext): Promise<DeliverResult> {
  ...
  const blocksTpl =
    platform === 'slack' ? String(ctx.mapping.blocks ?? '').trim() : ''
  if (!tpl.trim() && !blocksTpl) {
    return {
      ok: false,
      error: 'A message template or blocks JSON is required.',
    }
  }
  ...
  if (blocksTpl) {
    ...
    payloads.push({
      text: ctx.items.map((it) => resolveText(tpl, it)).join('\n'),
    })
  } else {
