...
async function mintToken(sa: ServiceAccount): Promise<string> {
  ...
  if (!res.ok || !json.access_token) {
    throw new Error(
      json.error_description || `Token request failed (HTTP ${res.status}).`,
    )
  }
...
async function test(ctx: DestinationTestContext): Promise<IntegrationResult> {
  ...
  if (!sa)
    return { ok: false, error: 'Paste a valid service-account JSON key.' }
  const spreadsheetId = String(ctx.config.spreadsheetId ?? '').trim()
  if (!spreadsheetId)
    return { ok: false, error: 'A spreadsheet id is required.' }
  try {
    ...
    return {
      ok: true,
      summary: `Connected to "${json.properties?.title ?? spreadsheetId}".`,
    }
  } catch (e) {
...
async function deliver(ctx: DeliverContext): Promise<DeliverResult> {
  ...
  const valueInputOption =
    ctx.config.valueInputOption === 'RAW' ? 'RAW' : 'USER_ENTERED'
  ...
  try {
    ...
    if (!res.ok)
      return {
        ok: false,
        error: json.error?.message || `Append failed (HTTP ${res.status}).`,
      }
