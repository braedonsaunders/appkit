...
let runtimeConfig: PdfRuntimeConfig = {}

export function configurePdfRuntime(config: PdfRuntimeConfig): void {
  if (browserPromise && config.executablePath !== runtimeConfig.executablePath) {
    throw new Error('PDF runtime executablePath cannot change after Chromium has started')
  }
...
function configuredStorageOrigin(): string {
  const endpoint =
    runtimeConfig.storageOrigin ?? process.env.APPKIT_STORAGE_ENDPOINT ?? 'http://localhost:9000'
  return configuredHttpOrigin(endpoint, 'APPKIT_STORAGE_ENDPOINT')
}

function configuredAppOrigin(): string {
  return configuredHttpOrigin(
    runtimeConfig.appOrigin ?? process.env.APPKIT_PUBLIC_APP_URL ?? 'http://localhost:3000',
    'APPKIT_PUBLIC_APP_URL',
  )
}
