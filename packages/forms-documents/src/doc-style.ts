...
const DOC_FG = color('fg')
const DOC_MUTED = color('fg-muted')
const DOC_BORDER = color('border-strong')
const DOC_SUBTLE = color('border-subtle')
const DOC_PRIMARY = color('primary')
...
export function documentBodyCss(sel = '.doc-body'): string {
  const s = sel
  return `
${s} { font-family: ${DOC_FONT}; font-size: ${DOC_FONT_SIZE_PX}px; line-height: ${DOC_LINE_HEIGHT}; color: ${DOC_FG}; word-wrap: break-word; }
...
${s} li { margin: 3px 0; }
${s} li > p { margin: 0; }
${s} blockquote { margin: 0 0 12px; border-left: 3px solid ${DOC_BORDER}; padding: 2px 14px; color: ${DOC_MUTED}; }
${s} table { border-collapse: collapse; width: 100%; margin: 0 0 12px; table-layout: fixed; }
${s} th, ${s} td { border: 1px solid ${DOC_BORDER}; padding: 5px 8px; vertical-align: top; }
${s} th { background: ${DOC_SUBTLE}; font-weight: 700; text-align: left; }
${s} img { max-width: 100%; height: auto; }
${s} a { color: ${DOC_PRIMARY}; text-decoration: underline; }
${s} pre { margin: 0 0 12px; background: ${DOC_FG}; color: ${DOC_BORDER}; padding: 10px; border-radius: 6px; overflow: auto; font-size: 12px; font-family: ui-monospace, SFMono-Regular, Menlo, monospace; }
${s} code { background: ${DOC_SUBTLE}; padding: 1px 4px; border-radius: 3px; font-size: 12px; font-family: ui-monospace, SFMono-Regular, Menlo, monospace; }
...
${s} hr { border: none; border-top: 1px solid ${DOC_BORDER}; margin: 16px 0; }
`
}
