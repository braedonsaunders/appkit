...
// @braedonsaunders/appkit-forms-core companion helpers (`_text`, `_image`, `_photos`, repeating-section and
...
const H1 = `font-size:21px;font-weight:800;color:${color('fg')};margin:0;letter-spacing:.2px;`
const SUB = `font-size:12px;color:${color('fg-muted')};margin:3px 0 0;`
const H2 =
  `font-size:13px;font-weight:700;color:${color('fg')};text-transform:uppercase;letter-spacing:.6px;margin:0;border-bottom:2px solid ${color('fg')};padding-bottom:3px;`
const TH =
  `text-align:left;border:1px solid ${color('border')};background:${color('border-subtle')};padding:5px 8px;font-size:10px;color:${color('fg-muted')};font-weight:700;text-transform:uppercase;letter-spacing:.3px;`
const TD =
  `border:1px solid ${color('border')};padding:5px 8px;font-size:11.5px;color:${color('fg')};vertical-align:top;`
const LBL =
  `width:18%;border:1px solid ${color('border')};background:${color('border-subtle')};padding:5px 8px;font-size:10.5px;font-weight:600;color:${color('fg-muted')};vertical-align:top;`
const VAL =
  `width:32%;border:1px solid ${color('border')};padding:5px 8px;font-size:11.5px;color:${color('fg')};vertical-align:top;`
...
function photoBlock(title: string, eachKey: string): string {
  return (
    ...
    `<img src="{{url}}" width="320" style="border:1px solid ${color('border')};border-radius:4px;display:block;" alt="" />` +
    `<div style="font-size:10px;color:${color('fg-muted')};padding-top:3px;">{{filename}}</div>` +
    `</td></tr></table>`
  )
}
...
export function generateFormPdfTemplate(
  ...
  blocks.push(
    `<div style="border-bottom:3px solid ${color('fg')};padding-bottom:8px;margin-bottom:12px;">` +
    ...
    for (const f of fields) {
      ...
      if (hasImageCompanion(f.type)) {
        kv.push(
          fullRow(
            label,
            `<img src="{{${f.id}_image}}" style="max-width:100%;max-height:220px;display:block;border:1px solid ${color('border')};border-radius:4px;" alt="" />`,
            `${f.id}_image`,
          ),
        )
        continue
  ...
  return {
    sourceHtml: `<div style="${FONT}color:${color('fg')};">${blocks.join('')}</div>`,
