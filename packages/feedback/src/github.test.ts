...
test('githubIssueBody merges default labels without duplicates', () => {
  assert.deepEqual(
    githubIssueBody({ title: 'Save fails', body: 'The save button does nothing.', labels: ['bug', 'Feedback'] }, ['feedback']),
    {
      title: 'Save fails',
...
test('createGithubIssuePublisher posts a sanitized issue through the host request', async () => {
  ...
  const publisher = createGithubIssuePublisher({
    ...
    request: async ({ url, method, headers, body }) => {
      ...
      if (method === 'GET') {
        return { status: 200, body: JSON.stringify({ items: [{ id: 3, number: 3, title: 'Known save failure', html_url: 'https://github.com/acme/product/issues/3' }] }) }
      }
      return { status: 201, body: JSON.stringify({ id: 99, number: 41, title: 'Save fails', html_url: 'https://github.com/acme/product/issues/41' }) }
    },
  })
