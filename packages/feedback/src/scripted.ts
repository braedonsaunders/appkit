  ...
  return {
    async send(input) {
      try {
        ...
        return {
          kind: 'unavailable',
          message: options.unavailableMessage ?? 'The report could not be completed. Try again in a moment.',
        }
      }
    },
  }
...
async function runScriptedTurn(
  options: ScriptedFeedbackOptions,
  input: FeedbackClientInput,
) {
  ...
  const help = options.knowledge && !input.forceFile && looksLikeHelp(text, input.answers)
  ...
  if (help.length > 0 && !input.forceFile) {
    ...
    return {
      ...
      title: first.title,
      explanation: first.excerpt?.trim() || 'This looks like a how-to question. The help article below should cover it.',
      help: help.slice(0, 3),
    }
  }
  ...
  if (options.publisher.searchOpen && !input.forceFile) {
    ...
    if (existing[0]) {
      ...
      return {
        ...
        explanation: 'An open product issue already describes this. You can follow that report instead of filing another.',
  ...
  return { kind: 'filed' as const, issue: { ...issue, title: issue.title || prepared.draft.title }, stripped: prepared.stripped }
}
