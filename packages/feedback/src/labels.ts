...
export const DEFAULT_FEEDBACK_LABELS: FeedbackLabels = {
  ...
  description: 'Describe what went wrong. We will check for a known answer, then file a product report if it is a defect.',
  close: 'Close',
  ...
  settingsDescription: 'People can report a product issue from any page. Filed issues go to the product tracker after personal data is removed.',
  ...
  searchDuplicatesHelp: 'When a matching open issue already exists, point the person to it instead of filing a duplicate.',
  save: 'Save',
  saving: 'Saving…',
}
...
export function workingStatusForTool(toolName: string | undefined, labels: FeedbackLabels = DEFAULT_FEEDBACK_LABELS): string {
  switch (toolName) {
    case 'search_help':
    case 'read_help':
