...
export function createGithubIssuePublisher(options: GithubIssuePublisherOptions): IssuePublisher {
  ...
  if (!REPO_RE.test(repo) || repo === '.' || repo === '..') throw new Error('A valid repository name is required.')
...
export function githubIssueBody(draft: IssueDraft, extraLabels: readonly string[] = []): {
  title: string
  body: string
  labels: string[]
} {
...
function readJson(response: { status: number; body: string }, failure: string): Record<string, unknown> {
  let parsed: unknown
  try {
  ...
  if (response.status < 200 || response.status >= 300) {
    ...
    const message = typeof record?.message === 'string' && record.message.trim() ? record.message : failure
