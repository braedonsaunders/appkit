...
import { GeneratedText, useGeneratedTranslations, GeneratedValue } from './generated-copy'
...
import { Input, Select, Textarea } from '@braedonsaunders/appkit-ui'
import type { FormField } from '@braedonsaunders/appkit-forms-core'
...
export function ElementPreview({
  ...
  if (t === 'divider') return <hr className="my-1 border-border" />
  return (
    <div className="space-y-1">
      <div className="text-xs font-medium text-fg-muted">
        <GeneratedValue value={label} />
        <GeneratedValue value={field.required ? <span className="text-danger"> *</span> : null} />
      </div>
...
function ElementInput({
  ...
  switch (t) {
        ...
        <div className="space-y-1">
          <GeneratedValue
            value={choices.slice(0, 4).map((l, i) => (
              <label key={i} className="flex items-center gap-2 text-xs text-fg-muted">
        ...
        <div className="flex gap-2">
          <GeneratedValue
            value={['Yes', 'No'].map((v) => (
              <span
                key={v}
                className="rounded-md border border-border px-3 py-1 text-xs text-fg-muted"
              >
        ...
        <div className="flex gap-1">
          <GeneratedValue
            value={['PASS', 'FAIL', 'N/A'].map((v) => (
              <span
                key={v}
                className="rounded border border-border px-2 py-1 text-[10px] text-fg-muted"
              >
                <GeneratedValue value={v} />
              </span>
            ))}
    ...
    case 'traffic_light':
      return (
        <div className="flex gap-1.5">
          <GeneratedValue
            value={['bg-success', 'bg-warning', 'bg-danger'].map((tone) => (
    ...
    case 'signature':
      return (
        <div className="flex h-16 items-center justify-center rounded border border-dashed border-border-strong bg-bg-subtle text-xs text-fg-subtle italic">
    ...
    case 'audio':
      return (
        <div className="flex h-16 items-center justify-center rounded border border-dashed border-border-strong bg-bg-subtle text-xs text-fg-subtle">
          <GeneratedText id="m_10e50435ff2693" />
        </div>
      )
    case 'sketch':
      return (
        <div className="flex h-20 items-center justify-center rounded border border-dashed border-border-strong bg-[repeating-linear-gradient(45deg,transparent,transparent_9px,rgba(148,163,184,0.12)_9px,rgba(148,163,184,0.12)_10px)] text-xs text-fg-subtle italic">
    ...
    case 'photo_ai':
      return (
        <div className="space-y-1.5">
          <div className="flex h-14 items-center justify-center rounded border border-dashed border-border-strong bg-bg-subtle text-xs text-fg-subtle">
            <GeneratedText id="m_18010438ac7907" />
          </div>
          <span className="inline-flex items-center gap-1 rounded-md border border-info bg-info-subtle px-2 py-1 text-[11px] text-info">
    ...
    case 'formula':
      return <Input disabled value="= computed" className="bg-bg-subtle italic" />
    case 'slider': {
      ...
      return (
        <div className="flex items-center gap-2">
          <input
            ...
            disabled
            className="h-2 flex-1 rounded-full bg-border accent-primary"
          />
    ...
    case 'gps':
      return (
        <div className="flex items-center gap-1.5 rounded border border-border bg-white px-2 py-1.5 text-xs text-fg-muted">
        ...
        <table className="w-full text-[10px]">
          <thead>
            <tr>
              ...
              <GeneratedValue
                value={scale.map((s) => (
                  <th key={s.value} className="px-1 text-center font-normal text-fg-subtle">
          ...
          <tbody>
            <GeneratedValue
              value={rows.slice(0, 3).map((r) => (
                <tr key={r.key}>
                  <td className="pr-1 text-fg-muted">
    ...
    case 'photo_annotated':
      return (
        <div className="space-y-1">
          <div className="relative flex h-24 items-center justify-center rounded border border-dashed border-border-strong bg-bg-subtle text-xs text-fg-subtle">
            <GeneratedText id="m_18010438ac7907" />
            <span className="absolute top-1/3 left-1/3 flex h-5 w-5 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-danger text-[10px] font-bold text-danger-fg ring-2 ring-surface">
              1
            </span>
            <span className="absolute top-1/2 left-2/3 flex h-5 w-5 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-danger text-[10px] font-bold text-white ring-2 ring-white">
              2
            </span>
          </div>
    ...
    case 'qr_scanner':
      return (
        <div className="flex items-center gap-2">
          ...
          />
          <span className="inline-flex items-center gap-1 rounded border border-border px-1.5 py-1 text-[11px] text-fg-muted">
        ...
        <ol className="space-y-1">
          <GeneratedValue
            value={itemsR.slice(0, 4).map((l, i) => (
              <li
                key={i}
                className="flex items-center gap-2 rounded border border-border bg-white px-2 py-1 text-xs"
              >
                <span className="w-4 text-center font-semibold text-fg-subtle">
    ...
    case 'rich_text':
      return (
        <div className="rounded border border-border">
          <div className="flex gap-2 border-b border-border bg-bg-subtle px-1.5 py-1 text-[11px] text-fg-subtle">
    ...
    case 'address':
      return (
        <div className="space-y-1.5">
          <div className="flex items-center gap-1 rounded border border-border bg-white px-2 py-1.5 text-xs text-fg-subtle">
            <MapPin size={12} /> <GeneratedText id="m_178de0c94da150" />
          </div>
          <div className="grid grid-cols-2 gap-1.5">
            <div className="col-span-2 h-7 rounded border border-border bg-white" />
            <div className="h-7 rounded border border-border bg-white" />
            <div className="h-7 rounded border border-border bg-white" />
          </div>
        </div>
      )
    ...
    case 'data_table': {
      ...
      return (
        <div className="overflow-hidden rounded border border-border text-[10px]">
          <div className="flex bg-bg-subtle">
            <GeneratedValue
              value={headers.slice(0, 5).map((h, i) => (
                <div
                  key={i}
                  className="flex-1 truncate border-r border-border px-1.5 py-1 font-medium text-fg-muted last:border-0"
                >
          ...
          <GeneratedValue
            value={[0, 1, 2].map((r) => (
              <div key={r} className="flex border-t border-border-subtle">
                <GeneratedValue
                  value={headers.slice(0, 5).map((_, i) => (
                    <div
                      key={i}
                      className="flex-1 border-r border-border-subtle px-1.5 py-1 text-fg-subtle last:border-0"
                    >
                      —
                    </div>
                  ))}
          ...
          <GeneratedValue
            value={
              sel && sel !== 'none' ? (
                <div className="bg-bg-subtle px-1.5 py-0.5 text-[9px] text-fg-subtle">
                  <GeneratedValue
                    value={
                      sel === 'single' ? (
    ...
    case 'metric': {
          ...
          <div className="flex h-14 items-end gap-1">
            <GeneratedValue
              value={[8, 14, 10, 16, 6].map((h, i) => (
                <span key={i} className="w-3 rounded-t bg-primary" style={{ height: h * 3 }} />
              ))}
            />
          </div>
        )
      ...
      return (
        <div className="inline-flex items-center gap-2 rounded-lg border border-border bg-white px-3 py-2">
          ...
          <div>
            <div className="text-2xl leading-none font-semibold text-fg">—</div>
            <div className="mt-0.5 text-[10px] tracking-wide text-fg-subtle uppercase">
...
function MiniTable({ field }: { field: FormField }) {
  ...
  if (cols.length === 0) {
    return (
      <div className="rounded border border-dashed border-border-strong p-2 text-center text-[11px] text-fg-subtle">
