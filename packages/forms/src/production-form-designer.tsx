...
type ProductionDesignerSchema = FormSchemaV1 & {
  workflow: NonNullable<FormSchemaV1['workflow']>
}

function normalizeDesignerSchema(schema: FormSchemaV1): ProductionDesignerSchema {
  return {
...
export function ProductionFormDesigner({
  ...
  const languages = (value: AppLocale) => localeLabels?.[value] ?? value.toUpperCase()
  const [schema, setSchema] = useState<ProductionDesignerSchema>(() => normalizeDesignerSchema(initialSchema))
  const [appName, setAppName] = useState(templateName)
  const [recordConfiguration, setRecordConfiguration] = useState<RecordConfig>(recordConfig ?? {})
  const [isPinned, setIsPinned] = useState(pinned)
  // Unified editor: left rail tab + right surface.
  const [leftTab, setLeftTab] = useState<
    'overview' | 'build' | 'record' | 'list' | 'actions' | 'assignments' | 'permissions'
  >('build')
  const [surface, setSurface] = useState<'build' | 'flows'>(
    initialSurface === 'flows' && renderFlows ? 'flows' : 'build',
  )
  ...
  function update(fn: (draft: ProductionDesignerSchema) => ProductionDesignerSchema) {
  ...
  function renameTab(id: string, title: string) {
    update((draft) => {
      ...
      if (t) t.title = writeText(t.title, title, contentLocale)
      return draft
    })
  }
  ...
  function publish() {
    ...
    start(async () => {
      ...
      if (publishedHref) adapter.navigate?.(publishedHref)
    })
  }
    ...
    <div className={`flex h-full min-h-0 flex-col ${className ?? ''}`}>
      <header className="flex shrink-0 items-center justify-between border-b border-border bg-surface px-4 py-2">
        <div className="flex min-w-0 items-center gap-3">
          <a href={backHref} className="shrink-0 text-sm text-primary hover:underline">
      ...
      <div className="flex min-h-0 flex-1 overflow-hidden">
        ...
        <aside className="flex w-1/3 min-w-72 max-w-[30rem] shrink-0 flex-col border-r border-border bg-surface">
              ...
              value={
                leftTab === 'overview' ? (
                  <OverviewPanel
                    ...
                    overview={overview}
                    onSaved={setAppName}
                    onSave={(input) => adapter.saveOverview({ templateId, ...input })}
                    canPin={canPin}
                    pinned={isPinned}
                    onPin={adapter.setPinned ? async (next) => {
                  ...
                  <RecordBehaviorPanel
                    value={recordConfiguration}
                    roles={roles}
                    onChange={setRecordConfiguration}
                    onSave={async (next) => {
                      const result = await adapter.saveRecordConfig({ templateId, recordConfig: next })
                      if (!result.ok) throw new Error(result.error ?? 'Record behavior could not be saved.')
                    }}
                  />
                ) : leftTab === 'list' ? (
                  <RecordListPanel
                    value={listConfig}
                    fields={listFields}
                    onChange={(list) => setRecordConfiguration((current) => ({ ...current, list }))}
                    onSave={async (list) => {
                      const result = await adapter.saveListConfig({ templateId, listConfig: list })
                      if (!result.ok) throw new Error(result.error ?? 'Record list could not be saved.')
                    }}
                  />
                ) : leftTab === 'actions' ? (
                  <RecordActionsPanel flows={flows} adapter={recordActionAdapter} />
                ) : leftTab === 'assignments' ? (
                  <AssignmentsPanel createHref={assignmentCreateHref} listHref={assignmentsHref} />
                ) : leftTab === 'permissions' ? (
                  <PermissionsPanel
                    roles={roles}
                    initial={allowedRoles}
                    onSave={(next) => adapter.savePermissions({ templateId, allowedRoles: next })}
                  />
                ) : (
                  <>
                    <button
                    ...
                    <h3 className="mb-1 text-[10px] font-semibold tracking-wider text-fg-muted uppercase">
        ...
        <div className="flex min-w-0 flex-1 flex-col bg-bg-subtle bg-bg">
              ...
              value={
                surface === 'build' ? (
                  <div className="ml-auto flex items-center gap-2">
                    <span
                      className="hidden text-[10px] font-semibold tracking-wider text-fg-muted uppercase sm:block"
            ...
            value={
              surface === 'flows' ? (
                <div className="min-h-0 flex-1">
                  {renderFlows?.({ templateId, name: appName, schema, flows })}
                </div>
              ) : (
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex flex-wrap items-center gap-1 rounded-lg border border-border bg-surface p-1.5">
                        ...
                        <button
                          type="button"
                          onClick={addTab}
                          className="flex items-center gap-1 rounded-md px-2.5 py-1 text-xs font-medium text-primary hover:bg-primary-subtle hover:bg-primary-subtle/40"
                                ...
                                <GeneratedValue
                                  value={
                                    sec.canvas ? (
                                      ...
                                      <div className="rounded-md border border-dashed border-border-strong p-4 text-center text-xs text-fg-muted border-border">
          ...
          value={
            selection.kind === 'field' && selectedField ? (
              <FieldProperties
                ...
                dataSourcesHref={dataSourcesHref}
                onChange={(patch) =>
      ...
      {renderAssistant?.({
        ...
        apply: (next) => {
          ...
          if (normalized.tabs?.length) setDesignerTab(normalized.tabs[0]!.id)
...
function FieldPaletteGroup({
  ...
  return (
    <div className="mb-3">
      <div className="px-1 pb-1 text-[10px] font-semibold tracking-wider text-fg-muted uppercase">
          ...
          value={group.types.map((t) => {
            ...
            return (
              <button
                ...
                onDragEnd={onDragEnd}
                onClick={() => onAdd(t)}
                className="flex cursor-grab items-center gap-2 rounded border border-border px-2 py-1 text-left text-xs hover:border-primary hover:bg-primary-subtle active:cursor-grabbing hover:bg-primary-subtle/40"
...
function IconButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className="rounded p-1 text-fg-muted hover:bg-surface-hover hover:text-fg disabled:cursor-not-allowed disabled:opacity-40"
    >
  ...
  return (
    <Reorder.Item
      ...
      className={`flex items-center justify-between gap-2 rounded px-1 py-2 ${
        isSelected ? 'bg-primary-subtle' : 'bg-surface'
      }`}
    >
      <button
        type="button"
        ...
        className="cursor-grab touch-none rounded p-0.5 text-fg-subtle hover:text-fg-muted active:cursor-grabbing"
      >
            ...
            value={steps.map((s, i) => (
              ...
              >
                <div className="flex items-start gap-2">
                  <span className="mt-1 inline-flex h-6 w-6 items-center justify-center rounded-full bg-primary-subtle text-xs font-semibold text-primary bg-primary-subtle/40">
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="space-y-1">
        ...
        <Input
          value={readText(field.label, locale)}
          placeholder={tGeneratedValue(localizeText(field.label, locale, field.id, defaultLocale))}
          onChange={(e) => onChange({ label: writeText(field.label, e.target.value, locale) })}
        />
      </div>
      <div className="space-y-1">
        <Label className="text-xs">
        ...
        <Textarea
          rows={2}
          value={readText(field.helpText, locale)}
...
function SliderConfigEditor({
  ...
  return (
    <div className="space-y-2 rounded-md border border-border p-2">
      <div className="text-[10px] font-semibold tracking-wider text-fg-muted uppercase">
...
type DsColumns = FormDataSource['columns']
...
function DataBindingEditor({
  ...
  sources: readonly FormDataSource[]
  loading: boolean
  ...
  return (
    <div className="space-y-2.5 rounded-md border border-primary bg-primary-subtle/30 p-2">
...
function LookupBindingFields({
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="space-y-1.5 rounded border border-border bg-surface p-2">
        <div className="flex items-center justify-between">
          <span className="text-[10px] font-semibold tracking-wider text-fg-muted uppercase">
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="space-y-1">
        ...
        <div className="flex flex-wrap gap-1.5 rounded border border-border bg-surface p-1.5">
          <GeneratedValue
            value={cols.map((c) => {
              ...
              return (
                <button
                  ...
                  className={`rounded px-1.5 py-0.5 text-[11px] ${
                    on ? 'bg-primary-subtle text-primary' : 'bg-surface-hover text-fg-muted line-through'
                  }`}
                >
...
function ChoiceOptionsEditor({
  ...
  return (
    <div className="space-y-2 rounded-md border border-border bg-bg-subtle/50 p-2">
              ...
              <GeneratedValue
                value={options.map((opt, i) => (
                  <li key={i} className="flex items-center gap-1">
                    ...
                    <Input
                      className="h-8 flex-1 text-xs"
                      value={readText(opt.label, locale)}
...
function TableConfigEditor({
  ...
  return (
    <div className="space-y-3 rounded-md border border-border bg-bg-subtle/50 p-2">
      <div className="text-xs font-semibold text-fg">
              ...
              <ul className="space-y-2">
                <GeneratedValue
                  value={columns.map((c, i) => (
                    <li key={i} className="space-y-1 rounded border border-border bg-surface p-2">
                      <div className="flex items-center gap-1">
                        <Input
...
function TableColumnOptions({
  ...
  const tGenerated = useGeneratedTranslations()
  return (
    <div className="space-y-1 rounded border border-border bg-bg-subtle p-1.5">
      <div className="text-[10px] font-semibold tracking-wider text-fg-muted uppercase">
        <GeneratedText id="m_0e69ebb67d27c2" />
      </div>
      <GeneratedValue
  ...
  return (
    <div className="space-y-3 text-sm">
      ...
      <div className="space-y-1">
        ...
        <Input
          value={readText(section.title, locale)}
          placeholder={tGeneratedValue(
            localizeText(section.title, locale, section.id, defaultLocale),
          )}
          onChange={(e) =>
            onChange({ title: writeText(section.title, e.target.value, locale) })
          }
        />
      </div>
      <GeneratedValue
      ...
      <div className="space-y-1">
        ...
        <Textarea
          rows={2}
          value={readText(section.description, locale)}
          placeholder={tGeneratedValue(
      ...
      <GeneratedValue
        value={
          section.repeating ? (
            <div className="space-y-2 rounded border border-border bg-bg-subtle/50 p-2">
              <div className="grid grid-cols-2 gap-2">
                <div className="space-y-1">
                  <Label className="text-xs">
                    <GeneratedText id="m_00c5b5a40a72f6" />
...
function Preview({
  ...
  return (
    <Card className="border-2 border-dashed border-border-strong bg-surface border-border">
      <CardHeader>
        <CardTitle className="text-base">
        ...
        <GeneratedValue
          value={(schema.workflow?.steps ?? []).map((step) => {
            ...
            return (
              ...
              >
                <div className="border-b border-border bg-bg-subtle px-3 py-1.5 text-xs font-semibold text-fg bg-bg-subtle/40">
...
function PreviewField({ type }: { type: FieldType }) {
  ...
  switch (type) {
    ...
    case 'signature':
      return <div className="h-16 rounded border border-dashed border-border-strong bg-bg-subtle" />
    case 'photo':
    case 'photo_upload':
    case 'file':
    case 'video':
    case 'audio':
      return (
        <div className="rounded border border-dashed border-border-strong p-3 text-center text-xs text-fg-muted">
        ...
        <div className="flex gap-1">
          <GeneratedValue
            value={['PASS', 'FAIL', 'N/A'].map((v) => (
              <span key={v} className="rounded border border-border px-2 py-1 text-[10px]">
        ...
        <div className="flex gap-1">
          <GeneratedValue
            value={[
              ...
              <span
                key={label as string}
                className="flex items-center gap-1 rounded border border-border px-2 py-1 text-[10px]"
              >
...
function OverviewPanel({
  ...
  onSave(input: { name: string; description: string; surfaceAsTool: boolean }): Promise<{ ok: boolean; error?: string }>
  canPin?: boolean
  pinned?: boolean
  onPin?: (next: boolean) => Promise<{ ok: boolean; error?: string }>
}) {
  const tGeneratedValue = useGeneratedValueTranslations()
  const tGenerated = useGeneratedTranslations()
              ...
              {onPin ? (
                <Button
                  ...
                  onClick={() => start(async () => {
                    ...
                    if (!result.ok) toast.error(tGeneratedValue(result.error ?? tGenerated('m_09249e0e6a801e')))
                  })}
                >
                  {pinned ? <PinOff size={14} /> : <Pin size={14} />}
                  {pinned ? <GeneratedText id="m_09eea65fa736d3" /> : <GeneratedText id="m_16b0d6e1ba7d5e" />}
                </Button>
              ) : null}
            </div>
          ) : null
...
function AssignmentsPanel({ createHref, listHref }: { createHref: string; listHref: string }) {
  return (
    <div className="space-y-3">
      <p className="text-xs text-fg-muted">
      ...
      <a href={createHref} className="block">
        <Button className="w-full">
          <Send size={14} /> <GeneratedText id="m_0b60657b166b0e" />
        </Button>
      </a>
      <a href={listHref} className="block">
          ...
          roles.length === 0 ? (
            ...
            <ul className="space-y-1">
              <GeneratedValue
                value={roles.map((r) => (
                  <li key={r.key}>
                    <label className="flex cursor-pointer items-center gap-2 rounded-md border border-border px-3 py-1.5 text-sm hover:bg-bg-subtle">
                      <input
                        type="checkbox"
