...
import {
  ...
  useProductionLocale as useLocale,
  useProductionTranslations as useTranslations,
  useProductionFormRuntime,
  type ProductionRuntimeLabels,
} from './production-runtime-context'
import type { GeneratedCopyTranslator } from './generated-copy'
import {
  AlertTriangle,
  Bold,
  Check,
...
} from '@braedonsaunders/appkit-forms-core/response-normalize'
import { sanitizeDocumentHtml } from '@braedonsaunders/appkit-forms-core/sanitize'
...
import type {
  ...
  ProductionDataQueryResult as DataQueryResult,
  ProductionDataRow as DataRow,
...
import { RiskMatrixField } from './risk-matrix'
import {
  ProductionFileUpload,
  dataUrlToFile,
  type ProductionAttachedFile as AttachedFile,
  type ProductionFileUploadProps,
} from './production-file-upload'
import { ProductionPremiumSection as PremiumSection } from './production-runtime-layout'
import { ProductionSection as Section } from './production-runtime-layout'
import { canvasCss, columnsCss, gridClass, resolveCanvas } from './canvas-runtime'
import { attachmentIdsEqual, singlePrimaryPhoto } from './photo-field-state'

function RawImage({ optimizationReason: _, ...props }: ImgHTMLAttributes<HTMLImageElement> & { optimizationReason?: string }) {
  return <img {...props} />
}

function FileUpload(props: Omit<ProductionFileUploadProps, 'requestUpload' | 'finalizeUpload'>) {
  const { adapter } = useProductionFormRuntime()
  if (!adapter.requestUpload || !adapter.finalizeUpload) {
    return (
      <Alert variant="destructive">
        <AlertDescription>File upload is not configured for this form runtime.</AlertDescription>
      </Alert>
    )
  }
  return (
    <ProductionFileUpload
      {...props}
      requestUpload={adapter.requestUpload}
      finalizeUpload={adapter.finalizeUpload}
    />
  )
}
 ...
 */
export function ProductionFormRenderer({
  adapter,
  locale,
  labels,
  translateGenerated,
  ...input
}: ProductionFormRendererProps) {
  return (
    <ProductionFormRuntimeProvider
      adapter={adapter}
      locale={locale}
      labels={labels}
      translateGenerated={translateGenerated}
    >
      <ProductionFormRendererBody {...input} />
    </ProductionFormRuntimeProvider>
  )
}

function ProductionFormRendererBody({
  templateId,
  templateName,
  version,
  schema,
  ...
  const { adapter } = useProductionFormRuntime()
  const createDraftResponse = (input: { templateId: string; complianceObligationId?: string | null }) =>
    adapter.createDraft({ templateId: input.templateId, obligationId: input.complianceObligationId })
  const saveFormResponseDraft = (input: {
    responseId: string
    ...
    baseRevision: number
  }) => adapter.saveDraft(input)
  const submitFormResponse = (input: {
    templateId: string
  ...
  useEffect(() => {
    function handleUnload() {
      ...
      try {
        ...
        if (adapter.draftBeaconUrl) navigator.sendBeacon?.(adapter.draftBeaconUrl, blob)
      } catch {
  ...
  if (field.type === 'formula' && field.formula) {
    ...
    return (
      <Input
        value={display}
        disabled
        className="bg-bg-subtle font-mono text-sm"
      ...
      return (
        <div className="flex items-center gap-3">
          <input
            ...
            className="h-2 flex-1 cursor-pointer appearance-none rounded-full bg-bg-subtle accent-primary"
          />
          <span className="w-12 text-right text-sm font-semibold text-fg tabular-nums">
            ...
            value={opts.map((o) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <GeneratedValue
                      value={sel ? <span className="h-2.5 w-2.5 rounded-full bg-primary" /> : null}
                    />
                  </span>
                ...
                rows.length === 0 ? (
                  <tr>
                    <td
                      ...
                      className="px-2 py-3 text-center text-xs text-fg-muted"
                    >
                          ...
                          rowMode === 'addable' ? (
                            <td className="px-1 py-1 text-center align-middle">
                              <button
                                ...
                                className="rounded p-1 text-fg-muted hover:text-danger disabled:cursor-not-allowed disabled:opacity-30"
                              >
                                <Trash2 size={14} />
                              </button>
                            </td>
            ...
            rowMode === 'addable' ? (
              <div className="border-t border-border p-1.5">
                <button
                  ...
                  className="flex items-center gap-1 rounded px-2 py-1 text-xs font-medium text-primary hover:bg-primary-subtle disabled:cursor-not-allowed disabled:opacity-40"
                >
  ...
  return (
    <div className="space-y-2">
      <div className="relative max-w-sm">
        <Search
          size={15}
          className="pointer-events-none absolute top-2.5 left-2.5 text-fg-muted"
        />
        <Input
          type="search"
          value={search}
            ...
            <GeneratedValue
              value={res.rows.map((r, i) => {
                ...
                return (
                  <tr
                    key={rowId}
                    className={`border-b border-border ${isSel ? 'bg-primary-subtle' : ''} ${
                      selectable !== 'none' ? 'cursor-pointer hover:bg-bg-subtle' : ''
                    }`}
                  ...
                  >
                    ...
                    <GeneratedValue
                      value={showCols.map((c) => (
                        <td key={c.key} className="px-2 py-1.5 text-fg">
                          <GeneratedValue
...
function MetricBlock({ field, evalCtx }: { field: FormField; evalCtx: EvalContext }) {
          ...
          <GeneratedValue
            value={res.groups.map((g) => (
              <div key={g.key} className="flex items-center gap-2 text-xs">
                <span className="w-28 truncate text-fg">
        ...
        <GeneratedValue
          value={res.groups.map((g) => (
            <div key={g.key} className="flex flex-1 flex-col items-center justify-end gap-1">
              ...
              />
              <span className="w-full truncate text-center text-[9px] text-fg-muted">
                <GeneratedValue value={g.key} />
              </span>
            </div>
          ))}
  ...
  return (
    <div className="inline-flex flex-col rounded-lg border border-border bg-surface px-4 py-3">
      <span className="text-3xl font-semibold text-fg tabular-nums">
        <GeneratedValue value={fmt(res?.value ?? null)} />
      </span>
      <span className="mt-0.5 text-[10px] tracking-wide text-fg-muted uppercase">
  ...
  return (
    <div className="space-y-2">
      ...
      <GeneratedValue
        value={
          a ? (
            <div className="space-y-2 rounded-lg border border-border bg-bg-subtle/60 p-3 text-sm">
              <GeneratedValue
                value={
                  a.summary ? (
                    <p className="text-fg">
              ...
              <GeneratedValue
                value={
                  a.ppe.length > 0 ? (
                    <div>
                      <div className="mb-1 flex items-center gap-1 text-xs font-semibold tracking-wide text-fg-muted uppercase">
                        ...
                        <GeneratedValue
                          value={a.ppe.map((p, i) => (
                            <li key={i} className="flex items-start gap-1.5">
                              <span
                                className={
                                  p.status === 'present' ? 'text-success' : 'text-danger'
                                }
                              >
    ...
    <ol className="space-y-1">
      <GeneratedValue
        value={ordered.map((v, i) => (
          ...
          >
            <span className="w-5 text-center font-semibold text-fg-muted">
              <GeneratedValue value={i + 1} />
            </span>
            ...
            <button
              ...
              disabled={i === 0}
              onClick={() => move(i, -1)}
              className="rounded p-1 text-fg-muted enabled:hover:bg-bg-subtle enabled:hover:text-fg disabled:opacity-30"
    ...
    <div className="space-y-2">
      <div className="relative">
        ...
        <GeneratedValue
          value={
            searching ? (
              <span className="absolute top-2.5 right-2 text-xs text-fg-muted">…</span>
            ) : null
          }
        />
        <GeneratedValue
                  ...
                  value={suggestions.map((s, i) => (
                    <li key={i}>
                      <button
                        type="button"
                        onClick={() => choose(s)}
                        className="block w-full px-2 py-1.5 text-left text-xs text-fg hover:bg-bg-subtle"
                      >
...
function SignatureField({
  ...
  async function persist(dataUrl: string | null) {
    ...
    if (!adapter.requestUpload || !adapter.finalizeUpload) return
...
function SaveStatus({
  ...
  if (status === 'pending') {
    return (
      <span className="inline-flex items-center gap-1 rounded-full bg-bg-subtle px-2 py-0.5 text-[11px] font-medium text-fg">
  ...
  if (status === 'error') {
    return (
      <button
        ...
        className="inline-flex items-center gap-1 rounded-full bg-danger-subtle px-2 py-0.5 text-[11px] font-medium text-danger ring-1 ring-danger hover:bg-danger-subtle"
      >
        <CloudOff size={11} />
  ...
  return (
    <span className="inline-flex items-center gap-1 rounded-full bg-success-subtle px-2 py-0.5 text-[11px] font-medium text-success ring-1 ring-success">
      <Check size={11} />
      <GeneratedValue value={label} />
    </span>
  )
