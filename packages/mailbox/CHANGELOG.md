...
- b826626: Add the `@braedonsaunders/appkit-mailbox/react` operator surface: `MailboxInbox` and
  ...
  follows `@braedonsaunders/appkit-notifications`' `NotificationInbox` layout, folder rail, row
  ...
  `react`, `react-dom`, `@braedonsaunders/appkit-ui`, and `lucide-react` are optional peers, so
