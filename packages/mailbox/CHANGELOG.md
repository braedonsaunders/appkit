...
- b826626: Add the `@braedonsaunders/mailbox/react` operator surface: `MailboxInbox` and
  ...
  follows `@braedonsaunders/notifications`' `NotificationInbox` layout, folder rail, row
  ...
  `react`, `react-dom`, `@braedonsaunders/ui`, and `lucide-react` are optional peers, so
