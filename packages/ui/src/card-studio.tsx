    ...
    <div className="grid min-h-0 flex-1 lg:grid-cols-[minmax(280px,380px)_minmax(0,1fr)]">
      <div className="app-scroll min-h-0 space-y-6 overflow-y-auto border-r border-border p-5">
        <section className="space-y-4">
          ...
          <div className="space-y-1.5"><Label>Source</Label><Select value={draft.query.source} onChange={changeSource} options={catalog.sources.map((item) => ({ value: item.key, label: item.label, hint: item.description }))} />{source?.description ? <p className="text-xs leading-relaxed text-fg-muted">{source.description}</p> : null}</div>
        </section>
        ...
        <StudioSection title="Group by" onAdd={() => { const field = fields.find((item) => item.canDimension ?? !['number', 'currency'].includes(item.semanticType)); if (field) updateQuery((query) => ({ ...query, dimensions: [...(query.dimensions ?? []), { field: field.key }] })) }}>
          {(draft.query.dimensions ?? []).map((dimension, index) => <div key={index} className="flex items-center gap-2"><Select className="min-w-0 flex-1" value={dimension.field} onChange={(field) => updateQuery((query) => ({ ...query, dimensions: (query.dimensions ?? []).map((item, itemIndex) => itemIndex === index ? { ...item, field } : item) }))} options={fields.filter((field) => field.canDimension ?? !['number', 'currency'].includes(field.semanticType)).map((field) => ({ value: field.key, label: field.label }))} />{fieldFromSource(source!, dimension.field)?.semanticType === 'date' ? <Select className="w-28" value={dimension.bin ?? ''} onChange={(bin) => updateQuery((query) => ({ ...query, dimensions: (query.dimensions ?? []).map((item, itemIndex) => itemIndex === index ? { ...item, bin: bin ? bin as typeof item.bin : undefined } : item) }))} options={[{ value: '', label: 'Exact' }, ...['day', 'week', 'month', 'quarter', 'year'].map((value) => ({ value, label: value }))]} /> : null}<IconButton label="Remove dimension" onClick={() => updateQuery((query) => ({ ...query, dimensions: (query.dimensions ?? []).filter((_, itemIndex) => itemIndex !== index) }))} /></div>)}
        </StudioSection>
