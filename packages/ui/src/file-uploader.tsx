      ...
      <div
        ...
        className={cn(
          ...
          dragOver
            ...
            : 'border-border-strong bg-bg-subtle text-fg-muted hover:border-primary/60 hover:bg-primary-subtle/50',
        )}
      >
        ...
        {!compact && hint ? (
          <span className="mt-1 text-[11px] text-fg-muted">{t(hint)}</span>
        ) : null}
        <input
          ref={inputRef}
          type="file"
      ...
      {items.length > 0 ? (
        <ul className="space-y-1">
          {items.map((item) => (
            <li
              key={item.id}
              className="flex items-center justify-between gap-3 rounded-md border border-border bg-surface px-3 py-1.5 text-xs "
            >
              ...
              {item.status === 'error' ? (
                <button
                  ...
                  className="rounded px-2 py-0.5 text-[11px] font-medium text-danger hover:bg-danger-subtle"
                >
                  {t('Dismiss')}
                </button>
              ) : null}
