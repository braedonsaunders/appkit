...
const SIGNATURE_INK_TOKEN = '--ch-signature-ink'
...
export function SignaturePad({
  ...
  const resizeCanvas = useCallback(async () => {
    ...
    ctx.lineCap = 'round'
    ctx.lineJoin = 'round'
    const ink = getComputedStyle(document.documentElement).getPropertyValue(SIGNATURE_INK_TOKEN)
    const inkColor = ink.trim() ? `rgb(${ink.trim().replace(/\s+/g, ' ')})` : 'currentColor'
    ...
    <div ref={wrapRef} className={cn('w-full', className)}>
      <div
        className={cn(
          ...
          // uses the stable document-ink token and the exported PNG is
          // dark-on-transparent for PDFs,
          // so a dark backing would hide the stroke while signing and viewing.
          'relative w-full overflow-hidden rounded-md border bg-[rgb(var(--ch-signature-surface))]',
          disabled
            ? 'border-border'
          ...
          <>
            ...
            {!hasInk && !disabled ? (
              <div className="pointer-events-none absolute inset-0">
                <div
                  className="absolute right-4 left-4 border-t border-dashed border-border"
                  style={{ top: `${(2 / 3) * 100}%` }}
                />
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="text-xs text-fg-subtle">{t('Sign here')}</span>
                </div>
              </div>
            ) : null}
          </>
