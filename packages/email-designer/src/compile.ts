...
// understands. Merge values are escaped at render time by @appkit/email-render,
// so a compiled design is safe to store and re-render without re-sanitizing.

import { expandRepeatMarkers, sanitizeEmailFragment, sanitizeEmailHtml } from '@appkit/email-render'
...
export type CompileEmailDesignOptions = {
   ...
   * `inlineEmailCss` from `@appkit/email-designer/inline` on the server. Omit it
