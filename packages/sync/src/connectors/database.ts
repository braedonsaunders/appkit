...
function withCursorPredicate(
  ...
  if (m.query && m.query.trim()) return `SELECT * FROM (${base}) AS appkit_src WHERE ${pred}`
...
export const databaseConnector: Connector = {
  ...
  async test(ctx) {
    ...
    try {
      ...
      await conn?.close().catch((error: unknown) => ctx.log('warn', `Database close failed: ${error instanceof Error ? error.message : String(error)}`))
    }
  },
